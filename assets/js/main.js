...
function loading_percent()
{
	...
	var n_percent = parseInt((contentLoaded/10)*100);
...
function genre_events(data_id)
{
	...
	var genre_name;
	if(data_id=='humor_fest') genre_name = 'Humor Fest';
	else if(data_id=='film_fest') genre_name = 'Film Fest';
	else if(data_id=='fine_arts') genre_name = 'Fine Arts';
	else genre_name = data_id.charAt(0).toUpperCase() + data_id.slice(1);

	if(!data['message'].hasOwnProperty(genre_name)){
		$('#genre .coming_soon').show();
	} else {
		$('#genre .coming_soon').hide();
		$('.genre_events_row .events').html('');
...
function event_data(event_index)
{
	...
	if(event_object['tagline'].toLowerCase() == "general")
	{
		event_object['tagline'] = "";
	}
	...
	if(event_object.hasOwnProperty('rules')){
		$('#rules, #rules_label').show();
...
{
	var sound_state = 0; // means OFF
	var entry_counter = true; // counter for checking whether user is entering the theater for first time.
	...
	$('#mute').click(function () { // will play audio
		$('#mute').hide();
		$('#unmute').show();
		ion.sound.play('back_music');
		sound_state = 1;
	});
	$('#unmute').click(function () { // will stop audio
		$('#mute').show();
		$('#unmute').hide();
	...
	counter.addEventListener('load', function() {
		...
		snap_counter.select('#Contact_us_x28_glow_x29_').attr({
			cursor:'pointer'
		});
		snap_counter.select('#contact_us').click(function () {
			window.open(window.location.href+'team');
		});
		snap_counter.select('#Contact_us_x28_glow_x29_').click(function () {
			window.open(window.location.href+'team');
		});
		snap_counter.select('#pronites').attr({
			cursor:'pointer'
		});
		snap_counter.select('#Pronites__x28_glow_x29_').attr({
			cursor:'pointer'
		});
		snap_counter.select('#Pronites__x28_glow_x29_').click(function () {
			show_modal('pronites');
		});
		...
		snap_counter.select('#Sponsors_x28_glow_x29_').attr({
			cursor:'pointer'
		});
		snap_counter.select('#sponsors').click(function () {
			window.open('http://www.springfest.in/sponsors');
		});
		snap_counter.select('#Sponsors_x28_glow_x29_').click(function () {
			window.open('http://www.springfest.in/sponsors');
		});
			...
			.on("enter leave", function (e) {
				...
				if(e.type == 'enter') {
					if(entry_counter == true) {
						...
						sound_state = 1;
						entry_counter = false;
					} else if(entry_counter == false) {
						if(sound_state == 0 && scroll_dir2== 'FORWARD') {
							$('#mute').show();
	...
	corridor.addEventListener('load', function() {
		...
		var p_type = ['dance', 'dramatics', 'music', 'humor_fest', 'film_fest', 'literary', 'quiz', 'fine_arts', 'social', 'informals'];
		var ceiling = "#Ceiling";
		var floor	= "#Floor";
	...
	var corridor_scene = new ScrollMagic.Scene({triggerElement: "#corridor_container", duration:2000,offset:(outer_ht/2 -36)})
			...
			.on("progress", function (e) {
				...
				if(corridor_load == true)
				{
					...
					if(scroll_dir3 == 'REVERSE') {
						$('#genre, #event').hide(400);
			...
			.on("progress", function (e) {
				...
				if(inner_theatre_load == true)
				{
					...
					if(progress >=0.93) {
						// scroll_dir3 = 'REVERSE';
						// console.log(scroll_dir3);
						var top = 6000+ (3*$('#outer_container').height()) + 2000*0.93;
